...
public:
    ...
    void primMST()
    {
        ...
        vector<psi> parent(num, make_pair("", -1)); 
        ...
        while (!pq.empty())
        {
            ...
            for (const auto &neighbor : adj_list[u])
            {
                ...
                if (!inMST[v] && weight < key[v])
                {
                    ...
                    parent[v] = make_pair(locations[u].getName(), v);
                }
            }
        }
    ...
    void dijkstra(string startLocation)
    {
        int startIndex = -1;
        for (int i = 0; i < locations.size(); ++i)
        {
        ...
        pq.push(make_pair(locations[startIndex].getName(), 0));
        dist[startIndex] = 0;
        ...
        while (!pq.empty())
        {
            ...
            for (auto &neighbor : adj_list[uIndex])
            {
                int neighborIndex = -1;
                for (int i = 0; i < locations.size(); ++i)
                {
                ...
                if (dist[uIndex] + weight < dist[v])
                {
                    dist[v] = dist[uIndex] + weight;
                    pq.push(make_pair(locations[v].getName(), dist[v]));
                }
            }
        }

        cout << "Shortest distances from " << locations[startIndex].getName() << ":" << endl;
        for (int i = 0; i < dist.size(); ++i)
        {
            cout << "To " << locations[i].getName() << " - Distance: " << dist[i] << endl;
        }
    }
};
...
int main()
{
    ...
    graph.link_location(2, 3, 8);
    graph.link_location(2, 4, 6);
    graph.link_location(2, 5, 9);

    graph.link_location(3, 4, 11);
    graph.link_location(3, 5, 13);

    graph.link_location(4, 5, 3);

    cout << "Running Dijkstra's Algorithm from 'Delivery Station (Karachi)':" << endl;
    graph.dijkstra("Delivery Station (Karachi)");
